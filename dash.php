...
if(isset($_SESSION['key'])){
  if(@$_GET['q']== 'addqns' && $_SESSION['key']=='sunny7785068889') {
      ...
      for($i=1;$i<=$n;$i++) {
          ...
          // Here, handle the image upload
          $target_dir = "uploads/"; // Specify the directory where uploaded images will be saved
          ...
          if (move_uploaded_file($_FILES["image".$i]["tmp_name"], $target_file)) {
              // Now, insert the question and image path into the database
              $q3=mysqli_query($con,"INSERT INTO questions (eid, qid, question, image_path, chapter, sequence) VALUES ('$eid','$qid','$qns', '$target_file', '$ch', '$i')");
              
              // Check if the query executed successfully
              if ($q3) {
                  echo "Image uploaded and question inserted into the database.";
              } else {
...
if(@$_GET['q']==4 && (@$_GET['step'])==2 ) {
  ...
  <div class="col-md-3"></div><div class="col-md-6"><form enctype="multipart/form-data" class="form-horizontal title1" name="form" action="update.php?q=addqns&n='.@$_GET['n'].'&eid='.@$_GET['eid'].'&ch=4 "  method="POST">
  <fieldset>';
  ...
  for($i=1;$i<=@$_GET['n'];$i++) {
      ...
      <!-- Image upload -->
      <div class="form-group">
        <label class="col-md-12 control-label" for="image'.$i.'">Upload Your Image</label>
        <div class="col-md-12">
